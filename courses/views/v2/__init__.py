...
from courses.utils import (
    active_contract_id_annotations,
...
class CourseViewSet(
    ...
    def get_queryset(self):
        ...
        # get_first_unexpired_b2b_run and get_filtered_runs match contracts in
        # Python over the loaded runs rather than with a b2b_contracts__in
        # filter - without the contract ids on hand each run costs a query.
        #
        # They come across as integer arrays rather than as a
        # Prefetch("b2b_contracts"). ContractPage is a Wagtail Page and a
        # ClusterableModel, and a prefetch without to_attr also builds a related
        # manager and a queryset clone per run: in production that pair cost
        # ~930ms of pure Python on a course with 149 runs, against 3-8ms for the
        # to_attr-style sibling prefetches over the same runs. The two methods
        # only ever read a contract's pk and organization_id, so arrays of those
        # carry everything and instantiate nothing.
        #
        ...
        # "b2b_contract" field renders the pk straight off b2b_contract_id. All
        # three aliases match cached_property fallbacks on CourseRun, so the
        # annotations shadow them.
