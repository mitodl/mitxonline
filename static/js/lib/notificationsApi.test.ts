...
describe("notifications API", () => {
  let helper: IntegrationTestHelper,
    getCookieStub: sinon.SinonStub,
    removeCookieStub: sinon.SinonStub

  beforeEach(() => {
    helper = new IntegrationTestHelper()
    getCookieStub = helper.sandbox.stub(Cookies, "get")
    removeCookieStub = helper.sandbox.stub(Cookies, "remove")
  })
  afterEach(() => {
    helper.cleanup()
  })
  ...
  describe("parseStoredUserMessage", () => {
    ...
    it("returns the correct message properties given an 'enroll failed' message cookie value", () => {
      assert.deepEqual(
        ...
        {
          text: `Something went wrong with your enrollment. Please contact support at ${SETTINGS.support_email}.`,
          type: "danger"
        }
      )
    })
