  ...
  ;[
    ...
    it(`allow users to course email subscription and handles ${returnedStatusCode} response`, async () => {
      ...
      const expectedUserMsgProps = success
        ? {
          type: ALERT_TYPE_SUCCESS,
          msg:  `You have been successfully subscribed to course ${enrollment.run.course_number} emails.`
        }
        : {
          type: ALERT_TYPE_DANGER,
          msg:  `Something went wrong with your request to course ${enrollment.run.course_number} emails subscription. Please contact support at ${SETTINGS.support_email}.`
        }
      helper.handleRequestStub
        .withArgs(`/api/enrollments/${enrollment.id}/`)
        .returns({
