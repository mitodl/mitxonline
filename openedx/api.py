...
def _create_edx_user_request(open_edx_user, user, access_token):  # noqa: C901
    ...
    max_attempts = settings.OPENEDX_SUGGESTED_USERNAME_MAX_ATTEMPTS
    attempt = 0
    suggested_usernames = []
    ...
    # we give ourselves a generous 120 seconds to accomplish this and we do not block if something else has the lock
    ...
    try:
        while attempt < max_attempts:
            ...
            # append to the list and only keep unique suggestions
            suggested_usernames = list(
                {*suggested_usernames, *data.get("username_suggestions", [])}
            )
...
def create_edx_user(user, edx_username=None):
    ...
    if open_edx_user.edx_username is None and edx_username is not None:
        if OpenEdxUser.objects.filter(edx_username=edx_username).exists():
            edx_username = _find_available_username(
                edx_username,
                model=OpenEdxUser,
...
def reconcile_edx_username(user):
    ...
    if not user.openedx_users.filter(edx_username__isnull=False).exists():
        ...
        edx_user.edx_username = usernameify(
