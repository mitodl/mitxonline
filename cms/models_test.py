...
def test_course_page_context(
    ...
    assert context == {
        ...
        "new_design": features.is_enabled("mitxonline-new-product-page", False, request.user.id if request.user.is_authenticated else "anonymousUser"),
        "new_footer": features.is_enabled("mitxonline-new-footer", False, request.user.id if request.user.is_authenticated else "anonymousUser"),
    }
