...
export function getStoredUserMessage(): UserMessage | null {
  const userMsgValue = Cookies.get(USER_MSG_COOKIE_NAME)
...
export function removeStoredUserMessage(): void {
  Cookies.remove(USER_MSG_COOKIE_NAME)
}
export const getNotificationAlertProps = (
  ...
  case ALERT_TYPE_SUCCESS:
    return {
      color: ALERT_TYPE_SUCCESS
    }

  case ALERT_TYPE_DANGER:
    return {
      color: ALERT_TYPE_DANGER
    }
