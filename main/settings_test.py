...
class TestSettings(TestCase):
    ...
    def test_s3_settings(self):
        ...
        settings_vars = self.patch_settings(
            {**REQUIRED_SETTINGS, "MITX_ONLINE_USE_S3": "False"}
        )
        self.assertNotEqual(
        ...
        with self.assertRaises(ImproperlyConfigured):
            self.patch_settings({"MITX_ONLINE_USE_S3": "True"})
    ...
    def test_admin_settings(self):
        ...
        settings_vars = self.patch_settings(
            {**REQUIRED_SETTINGS, "MITX_ONLINE_ADMIN_EMAIL": ""}
        )
        self.assertFalse(settings_vars.get("ADMINS", False))

        test_admin_email = "cuddle_bunnies@example.com"
        settings_vars = self.patch_settings(
            {**REQUIRED_SETTINGS, "MITX_ONLINE_ADMIN_EMAIL": test_admin_email}
        )
    ...
    def test_db_ssl_enable(self):
        ...
        settings_vars = self.patch_settings(
            {**REQUIRED_SETTINGS, "MITX_ONLINE_DB_DISABLE_SSL": "True"}
        )
        self.assertEqual(settings_vars["DATABASES"]["default"]["OPTIONS"], {})

        # Disable it
        settings_vars = self.patch_settings(
            {**REQUIRED_SETTINGS, "MITX_ONLINE_DB_DISABLE_SSL": "False"}
        )
        self.assertEqual(
            settings_vars["DATABASES"]["default"]["OPTIONS"], {"sslmode": "require"}
        )
