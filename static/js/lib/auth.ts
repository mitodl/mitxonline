...
import { has, includes, isEmpty } from "ramda"
import { routes } from "../lib/urls"
import { AuthResponse, AuthStates } from "../types/auth"
...
export const ALL_STATES: Array<AuthStates> = [
  STATE_ERROR,
...
export const generateLoginRedirectUrl = ({
  ...
  return `${routes.login.begin}?${qs.stringify({
    next
  })}`
}

export type StateHandler = (response: AuthResponse) => void
export type StateHandlers = Partial<Record<AuthStates, StateHandler>>
...
export const handleAuthResponse = (
  ...
  const { state, redirect_url, partial_token, errors, field_errors } =
    response || {}
  ...
  if (state === STATE_SUCCESS) {
    // window.location is intentional here to force a full page reload
    window.location.assign(redirect_url || routes.dashboard)
