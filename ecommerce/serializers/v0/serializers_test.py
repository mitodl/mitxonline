...
def test_basket_serializer_hides_a_zero_amount_discount(user):
    """A percent-off or dollars-off discount of 0 subtracts nothing to show."""
    ...
    BasketDiscount.objects.create(
        ...
        redeemed_discount=DiscountFactory.create(
            amount=0, discount_type=DISCOUNT_TYPE_DOLLARS_OFF
        ),
...
def test_basket_serializer_hides_a_linked_purchase_discount(user):
    """
    A linked-purchase discount's stored amount is always 0 and its real
...
def test_basket_serializer_shows_a_fixed_price_discount_of_zero(user):
    """
    A fixed-price discount sets the price instead of reducing it, so an amount
    of 0 is a real "free" discount rather than the no-op that 0 means for the
    subtractive types.
    """
    basket = BasketFactory.create(user=user)
    BasketItemFactory.create(basket=basket)
...
def test_basket_serializer_shows_a_discount_on_an_empty_basket(user):
    """
    A discount is judged on its own shape, so an applied code stays listed even
    before anything is in the basket to apply it to.
    """
    basket = BasketFactory.create(user=user)
    BasketDiscount.objects.create(
        redemption_date=now_in_utc(),
